...
> 않았다. Task 7–9 source는 구현되었지만 elevated Windows PowerShell `5.1`
> synthetic suite의 `failures=0` 증거가 없다. 아래 `[READY NOW]` 검증까지만
...
| 실제 server maintenance | `[IMPLEMENTATION BLOCKED]` | 위 gate와 별도 사용자 승인 |
...
- 개발 담당자는 macOS에서 tracked-clean handoff ZIP과 sidecar를 만든다.
- Windows operator는 checksum을 확인하고 synthetic suite를 실행한다.
...
- synthetic/real Windows test transcript
...
## `[READY NOW]` 1. Windows synthetic suite
...
## `[READY NOW]` 3. Windows handoff 검증

Windows에서 다음을 완료하기 전에는 handoff 내부 PowerShell을 실행하지 않는다.
...
- elevated Windows PowerShell `5.1` suite `failures=0`
...
포트 격리는 요구하지 않는다. 대신 같은 포트를 순차 사용하며 production
자동 재시작이 꺼져 있고 외부 ingress가 차단되어 있어야 한다.
