...
Task 11A.1–11A.6의 prepare, invoke, restore 구현은 존재하지만 Windows
PowerShell `5.1` 동작 검증은 아직 남아 있다. 이후 Task 7 private evidence
export, Task 8 결정론적 handoff, Task 9 전체 합성 lifecycle이 차례로 완료되어야
실제 서버 점검을 승인할 수 있다.
...
현재 `[READY NOW]` 범위는 별도 Windows 작업 복사본에서 Task 1–6 합성
PowerShell suite를 실행하는 것뿐이다. 이 범위는 실제 server root, save,
UE4SS 설치를 사용하지 않는다.

미구현 명령은 실행 가능한 PowerShell block으로 제공하지 않는다. Task 7–9와
Windows `failures=0` gate가 모두 통과한 뒤에만 실제 server command를
`[APPROVAL REQUIRED]` 절차로 승격한다.
...
4. elevated Windows PowerShell `5.1` suite `failures=0`
5. 별도 승인된 실제 Windows maintenance run
6. 개발 호스트에서 private ZIP/sidecar 재검증
7. 결과 검토 후 Task 11B 설계
...
- Windows PowerShell `5.1` 합성 suite가 `failures=0`을 출력한다.
