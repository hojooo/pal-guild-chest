...
- Approval accepts exactly world ID, positive exact revision, Task 5 `audit.checksum`, approved slot candidate, and fixed deployment profile. Define `LP(s)=decimal_UTF8_byte_length(s)..":"..s`; hash the direct concatenation `LP("cgce.operator-approval.v1"), LP("world_id"), LP(world_id), LP("game_revision"), LP(canonical_decimal_revision), LP("audit_checksum"), LP(audit_checksum), LP("requested_target_slots"), LP(canonical_decimal_target), LP("deployment_profile"), LP(profile)` with no extra separators. The fixed vector for `world/alpha`, revision `12345`, 64 `a` characters, target `358`, and the fixed profile is `36a4cf7f20cb58e92ba1072b95a4b9c9986cd6f746ac580d77a44199e1b9ab76`. Verify lowercase SHA-256 with constant-time comparison and never echo a token. `require_operator_approval=false` grants no authority: every eventual apply still requires this token.
...
- Logger formatting performs no I/O, recursively redacts normalized password/token/authorization/cookie/secret/credential/API-key/private-key variants, escapes controls so text/JSONL remain one physical line, and rejects `granularity="slot"`, `slot_index`, or a `slots` payload at INFO-or-higher while allowing aggregate slot counts. Above the fixed 16 KiB limit it emits a valid replacement with `dropped=true`, format, original byte size, and `original_event`: copy the sanitized event name only at ≤256 UTF-8 bytes, otherwise use `event_name_omitted`; never byte-truncate.
...
- `command_router.new` accepts only five read-only ports for status, audit, guild listing, live verification, and existing report path. Parse one control-free line as exact lowercase `cgce <command>` with no shell/quote interpretation or arguments. Every successful response contains only the detached revision, mode, target, and state envelope plus its command payload; status-port extras such as approval tokens are never copied. `cgce apply` (including any attempted arguments) returns `MUTATION_BUILD_UNAVAILABLE` without calling any port, regardless of mode, token, or state. Other command errors use `CGCE-CMD-*` codes.
- `scheduler.retry` receives explicit positive `max_attempts`, nonnegative finite delay, injected schedule/cancel callbacks, optional `on_terminal(result)`, and a probe; it returns a non-table generation-guarded opaque controller accessed only through `scheduler.status(handle)` and idempotent `scheduler.cancel(handle)`, so raw field shadowing cannot forge readiness. The first probe runs immediately, so status distinguishes `PENDING` from terminal `READY|EXHAUSTED|FAILED|CANCELLED`; at most one timer is outstanding. Probe/callback errors use `CGCE-SCHED-*`, late callbacks are no-ops, forged handles fail closed, and even a synchronous fake scheduler cannot exceed the bound or deliver terminal twice.
- `scheduler.rescan_interval(nil)` is 60 seconds and rejects values below 30. `scheduler.cache_decision` always requests live inspection at startup, even for a matching completed cache. Cache/live signatures use exact `game_revision`, `target_slots`, lowercase SHA-256 `item_fingerprint`, and `owner_guild_id`; fallback may skip only when all exactly match. Unknown/malformed/missing data inspects conservatively and multiple drift reasons are deterministic.
