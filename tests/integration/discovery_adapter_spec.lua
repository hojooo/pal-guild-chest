...
describe("ue4ss_adapter narrow function observation", function()
    ...
    it("uses post-only registration for non-/Script/ functions and records observer failures privately", function()
        ...
        local secondary = ue4ss_adapter.observe_function(adapter, descriptor, function()
            return nil, "secondary observer secret at 0xC0FFEE"
        end)
        fake.fire(descriptor.path, "post", raw_function)
        local secondary_status = ue4ss_adapter.observation_status(adapter, secondary)
        a.equal("FAILED", secondary_status.state)
        a.equal("CGCE-UE4SS-OBSERVER-CALLBACK", secondary_status.errors[1].code)
        local secondary_encoded = json.encode(secondary_status)
        a.equal(nil, secondary_encoded:find("C0FFEE", 1, true))
        a.equal(nil, secondary_encoded:find("secret", 1, true))
        ...
        a.equal("CLOSED", ue4ss_adapter.observation_status(adapter, secondary).state)
        assert_zero_write_calls(fake.counters())
    end)
    ...
    it("invalidates before reverse cleanup, preserves both hook IDs, and continues after failures", function()
        ...
        a.equal("[]", json.encode(second_errors))
        a.equal(3, #fake.unregister_log())
        assert_zero_write_calls(fake.counters())
    end)

    it("fails closed on partial hook IDs and cleans up valid IDs after register secondary errors", function()
        ...
        install_exact(partial_fake, descriptor)
