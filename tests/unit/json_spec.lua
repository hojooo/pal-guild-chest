...
describe("json.array", function()
    it("constructs empty and populated arrays for deterministic encoding", function()
        a.equal("[]", json.encode(json.array()))
        a.equal('["first","second"]', json.encode(json.array({ "first", "second" })))
    end)

    it("does not change decoded empty-array behavior", function()
        a.equal("[]", json.encode(json.decode("[]")))
    end)
end)
